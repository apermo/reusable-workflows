...
- `reusable-stale.yml` no longer closes anything: the `days-before-issue-close` and
  `days-before-pr-close` defaults are now `-1`, which is `actions/stale`'s value for "never close".
  Inactive issues and PRs are still labelled `stale` on the same schedule (60 days for issues, 30
  for PRs), they just stay open until someone acts on them. Callers that want the previous
  behaviour can opt back in by passing `days-before-issue-close: 30` and
  `days-before-pr-close: 30` explicitly.
...
[Unreleased]: https://github.com/apermo/reusable-workflows/compare/v0.11.2...HEAD
